...
export const Goals: React.FC = () => {
  ...
  const categories = ['Savings', 'Travel', 'Technology', 'Education', 'Health', 'Investment', 'Emergency', 'Other'];
  ...
  const handleAddGoal = () => {
    if (!newGoal.title || !newGoal.target || !newGoal.deadline || !newGoal.category) return;
  ...
  const getGoalStatus = (goal: any) => {
    ...
    if (daysLeft < 0) return { status: 'overdue', color: 'red', icon: AlertTriangle };
    if (daysLeft <= 30) return { status: 'urgent', color: 'yellow', icon: Clock };
  ...
  return (
    <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center space-x-1 mt-1">
                ...
                <span className="text-sm text-green-600">
                  {goals.length > 0 ? ((completedGoals / goals.length) * 100).toFixed(1) : 0}% complete
                </span>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">${totalGoalTarget.toFixed(2)}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center space-x-1 mt-1">
                ...
                <span className="text-sm text-blue-600">${totalGoalProgress.toFixed(2)} saved</span>
              </div>
            </div>
            <div className="p-3 bg-blue-100 rounded-lg">
            ...
            <div key={goal.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => setEditingGoal({...goal, target: goal.target.toString()})}
      ...
      {editingGoal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Edit Goal</h3>
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    ${contributionModal.current.toFixed(2)} / ${contributionModal.target.toFixed(2)}
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                  <div 
                    className="bg-blue-500 h-2 rounded-full"
                    style={{ width: `${Math.min((contributionModal.current / contributionModal.target) * 100, 100)}%` }}
                  />
                </div>
              </div>
              ...
              <div className="bg-blue-50 rounded-lg p-3">
                <p className="text-sm text-blue-700">
                  ...
                  <span className="font-medium">
                    ${(contributionModal.current + (parseFloat(contribution) || 0)).toFixed(2)}
                  </span>{' '}
                  towards your goal.
                </p>
              </div>
