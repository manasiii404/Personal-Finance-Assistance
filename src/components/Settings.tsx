...
export const Settings: React.FC = () => {
  ...
  const handleExportData = () => {
    ...
    a.href = url;
    a.download = `finance-ai-settings-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
    ...
    switch (activeTab) {
            ...
            <div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  { key: 'sms', label: 'SMS Notifications', icon: Smartphone, description: 'Get text message alerts' },
                  { key: 'push', label: 'Push Notifications', icon: Bell, description: 'Browser push notifications' },
                  { key: 'whatsapp', label: 'WhatsApp Notifications', icon: MessageSquare, description: 'WhatsApp message alerts' }
            ...
            <div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  { key: 'goalReminders', label: 'Goal Reminders', description: 'Regular updates on goal progress' },
                  { key: 'weeklyReports', label: 'Weekly Reports', description: 'Weekly spending summaries' },
                  { key: 'monthlyReports', label: 'Monthly Reports', description: 'Comprehensive monthly reports' }
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <button
                    onClick={() => setSecurity({...security, biometricAuth: !security.biometricAuth})}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${
      ...
      case 'integrations':
        return (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">MCP Integration Settings</h3>
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-200 mb-6">
                <h4 className="font-medium text-blue-900 mb-2">Multi-Channel Protocol (MCP)</h4>
                <p className="text-sm text-blue-700">
                  MCP enables real-time financial monitoring and intelligent automation across multiple channels.
                </p>
              </div>

              <div className="space-y-4">
                {[
                  { key: 'bankSync', label: 'Bank Account Sync', description: 'Automatically sync transactions from bank accounts' },
                  { key: 'smsParser', label: 'SMS Transaction Parser', description: 'Parse bank SMS notifications for transactions' },
                  { key: 'autoCategories', label: 'Auto-Categorization', description: 'Automatically categorize transactions using AI' },
                  { key: 'realTimeAlerts', label: 'Real-time Alerts', description: 'Instant notifications for financial activities' }
                ].map(({ key, label, description }) => (
                  <div key={key} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div>
                      <h5 className="font-medium text-gray-900">{label}</h5>
                    ...
                    <div className="flex items-center space-x-3">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        {integrations[key as keyof typeof integrations] ? 'Active' : 'Inactive'}
                      </span>
                      <button
                        onClick={() => setIntegrations({...integrations, [key]: !integrations[key as keyof typeof integrations]})}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${
                          integrations[key as keyof typeof integrations] ? 'bg-blue-600' : 'bg-gray-300'
                        }`}
                      >
                        <span
                          className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${
                            integrations[key as keyof typeof integrations] ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {[
                  { name: 'Chase Bank', type: 'Checking', status: 'Connected', lastSync: '2 minutes ago' },
                  { name: 'Wells Fargo', type: 'Savings', status: 'Connected', lastSync: '5 minutes ago' },
                  { name: 'American Express', type: 'Credit Card', status: 'Connected', lastSync: '1 hour ago' }
                ].map((account, index) => (
                  <div key={index} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{account.type} • Last sync: {account.lastSync}</p>
                      </div>
                    </div>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h5 className="font-medium text-yellow-900">Data Retention</h5>
                      <p className="text-sm text-yellow-700 mt-1">
                        Your financial data is stored securely and retained according to your preferences.
                        You can configure retention periods for different types of data.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div>
              ...
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                ...
                <div className="text-sm text-blue-700 space-y-1">
                  <p>• All financial data is encrypted at rest and in transit</p>
                  <p>• MCP protocols ensure secure multi-channel communication</p>
